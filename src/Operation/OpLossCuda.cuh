...
template <typename T>
Tensor mseLossOpCudaImpl(const Tensor& input, const Tensor& target, LossReduction reduction = LossReduction::MEAN) {
  TensorIteratorCuda iterator(input, target);
  auto outShape = iterator.setupBroadcast();
  ASSERT(iterator.isBroadcastOk());
  Tensor out(outShape, input.options().noGrad());
  iterator.template forEach<T>(out, [] __device__(const T& a, const T& b) -> T { return (a - b) * (a - b); });
  switch (reduction) {
...
template <typename T>
Tensor mseLossOpBackwardCudaImpl(const Tensor& grad, const Tensor& input, const Tensor& target,
                                 LossReduction reduction = LossReduction::MEAN) {
  TensorIteratorCuda iterator(input, target, grad);
  auto outShape = iterator.setupBroadcast();
  ASSERT(iterator.isBroadcastOk());
  Tensor out(outShape, input.options().noGrad());
  iterator.template forEach<T>(out, [] __device__(const T& a, const T& b, const T& g) -> T { return 2 * (a - b) * g; });
...
template <typename T>
Tensor nllLossOpCudaImpl(const Tensor& input, const Tensor& target, LossReduction reduction = LossReduction::MEAN) {
  ASSERT(target.dim() == 1);
...
template <typename T>
Tensor nllLossOpBackwardCudaImpl(const Tensor& grad, const Tensor& input, const Tensor& target,
                                 LossReduction reduction = LossReduction::MEAN) {
  ASSERT(target.dim() == 1);
  ASSERT(target.dtype() == DType::Int64);
  auto batchSize = input.shape(0);
